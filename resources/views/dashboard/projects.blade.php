    ...
    <main class="flex-1 overflow-auto p-6 md:p-10">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                @endphp
                <div class="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden group hover:border-pink-500/30 transition-all flex flex-col shadow-2xl relative">
                    @if(count($imgs) > 0)
                    ...
                    <div class="p-8 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto flex items-center justify-between pt-6 border-t border-white/5">
                            ...
                            <div class="flex items-center space-x-2.5">
                                    ...
                                    @endphp
                                    <button 
                                        class="edit-btn w-10 h-10 flex items-center justify-center bg-pink-600/10 text-pink-400 rounded-xl hover:bg-pink-600 hover:text-white transition-all border border-pink-500/20 active:scale-95"
                                        data-id="{{ $project->id }}"
    ...
    <div id="newProjectModal" class="fixed inset-0 z-50 hidden bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] p-8 animate-in fade-in zoom-in duration-300">
            ...
            <form id="projectForm" action="{{ route('dashboard.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            ...
                            <input type="text" name="name" id="form_name" value="{{ old('name') }}" required placeholder="e.g. Nexus OS" 
                                   class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-white placeholder:text-slate-800 transition-all">
                        </div>
                        
                        <div>
                            <label class="block text-[9px] font-black text-pink-400 uppercase tracking-widest mb-2">Service</label>
                            <select name="type" id="form_type" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-white appearance-none transition-all">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="form_description" rows="2" placeholder="Technical summary..." 
                                  class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-white resize-none placeholder:text-slate-800 leading-relaxed transition-all">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <label class="flex items-center justify-between w-full p-4 border border-slate-800 border-dashed rounded-xl cursor-pointer bg-slate-950 hover:bg-slate-900 transition-all group/file">
                            <div class="flex items-center space-x-3">
                                <svg class="w-5 h-5 text-slate-600 group-hover/file:text-pink-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                ...
                <button type="submit" class="w-full bg-pink-600 hover:bg-pink-500 text-white font-black py-4 rounded-xl shadow-2xl shadow-pink-600/20 transition-all active:scale-95 text-[10px] uppercase tracking-[0.2em] flex items-center justify-center space-x-2">
    ...
    <div id="profileModal" class="fixed inset-0 z-50 hidden bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-8">
            ...
            <form action="{{ route('dashboard.profile.update') }}" method="POST" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-pink-600 hover:bg-pink-500 py-3 rounded-xl font-bold transition-all shadow-lg">Sync Identity</button>
            </form>
        </div>
    </div>
