...
<body class="h-full flex bg-white dark:bg-slate-950 text-slate-200 overflow-hidden">
    ...
    <aside class="w-72 bg-slate-100 dark:bg-slate-900 border-r border-pink-200 dark:border-slate-800 flex flex-col hidden md:flex">
        <div class="p-4 border-b border-pink-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <form action="{{ route('chat.new') }}" method="POST">
                @csrf
                <button type="submit" class="p-2 hover:bg-pink-100 dark:hover:bg-slate-800 rounded-lg text-slate-700 dark:text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-all" title="New Chat">
        ...
        <div class="p-4 border-t border-pink-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/50">
            <div class="flex items-center space-x-3 p-2 rounded-xl bg-slate-800/30 border border-pink-200 dark:border-slate-800">
                <div class="w-9 h-9 rounded-lg bg-pink-600 flex items-center justify-center text-slate-900 dark:text-white text-xs font-black shadow-lg shadow-pink-600/20 uppercase">
    ...
    <main class="flex-1 flex flex-col relative h-full">
        ...
        <nav class="h-16 bg-white dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-900/50 flex items-center justify-between px-6 sticky top-0 z-30">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative">
                    <button id="accountBtn" class="flex items-center justify-center w-8 h-8 bg-slate-800/50 border border-pink-200 dark:border-slate-800 rounded-full hover:border-pink-500/30 transition-all focus:outline-none overflow-hidden group">
                        @if(Auth::user()->profile_photo_url)
                            <img src="{{ asset(Auth::user()->profile_photo_url) }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-pink-600 flex items-center justify-center text-slate-900 dark:text-white text-[10px] font-black uppercase">
                                {{ substr(Auth::user()->name, 0, 1) }}
                            </div>
                        @endif
                    </button>
                    <!-- Simple dropdown for chat page -->
                    <div id="accountDropdown" class="absolute right-0 mt-3 w-48 bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 rounded-2xl shadow-2xl opacity-0 invisible transition-all z-50 p-2">
    ...
    <div id="notifModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/80 backdrop-blur-md transition-all duration-300">
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-lg rounded-3xl shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="notifContainer">
                <div class="p-6 border-b border-pink-200 dark:border-slate-800 flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="w-11 h-11 rounded-2xl bg-pink-600/10 flex items-center justify-center text-pink-400">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
                        </div>
                        <div>
                            <h2 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tighter">Console Notifications</h2>
                            <p class="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Latest Broadcasts</p>
                        </div>
                    </div>
                    <button id="closeModal" class="p-2 hover:bg-pink-100 dark:hover:bg-slate-800 rounded-xl text-slate-500 hover:text-slate-900 dark:text-white transition-colors">
