...
<body class="h-full flex bg-white dark:bg-white text-slate-900 dark:text-slate-900 overflow-hidden">
    ...
    <aside class="w-72 bg-slate-950 border-r border-white/10 flex flex-col hidden md:flex">
        <div class="p-4 border-b border-white/10 flex items-center justify-between">
            <a href="{{ route('dashboard') }}" class="flex items-center space-x-2 text-pink-400 hover:text-pink-300 transition-colors">
            ...
            <form action="{{ route('chat.new') }}" method="POST">
                @csrf
                <button type="submit" class="p-2 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-all" title="New Chat">
            ...
            @if(count($threads) > 0)
                @foreach($threads as $thread)
                    <a href="{{ route('dashboard.chat', ['bot' => $bot_id, 'chatId' => $thread->chat_id]) }}" 
                       class="block px-3 py-3 rounded-xl hover:bg-white/5 cursor-pointer text-sm transition-all {{ $current_chat_id == $thread->chat_id ? 'bg-pink-600/10 text-pink-500 border border-pink-500/20 shadow-lg' : 'text-slate-400 hover:text-white' }}">
                ...
                @endforeach
            @else
                <div class="px-3 py-10 text-center text-xs text-slate-700 dark:text-slate-600 italic">No history found</div>
            @endif
        </div>

        <div class="p-4 border-t border-white/10 bg-slate-900/50">
            <div class="flex items-center space-x-3 p-2 rounded-xl bg-slate-950 border border-white/10">
    ...
    <main class="flex-1 flex flex-col relative h-full">
        ...
        <nav class="h-16 bg-slate-950 backdrop-blur-md border-b border-white/10 flex items-center justify-between px-6 sticky top-0 z-30">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative">
                    <button onclick="document.getElementById('clockModal').classList.remove('hidden')" class="p-2.5 text-white hover:text-pink-400 transition-colors focus:outline-none bg-white/5 rounded-lg border border-white/5 group">
                ...
                <div class="relative">
                    <button id="accountBtn" class="flex items-center justify-center w-8 h-8 bg-slate-800/50 border border-white/10 rounded-full hover:border-pink-500/30 transition-all focus:outline-none overflow-hidden group">
                    ...
                    <div id="accountDropdown" class="absolute right-0 mt-3 w-48 bg-slate-950 border border-white/10 rounded-2xl shadow-2xl opacity-0 invisible transition-all z-50 p-2">
                ...
                @if(count($chat_history) == 0)
                    <div id="emptyState" class="flex flex-col items-center justify-center space-y-6 opacity-30 mt-20">
                        ...
                        <div class="text-center">
                            <h2 class="text-2xl font-black text-slate-700 dark:text-slate-600 dark:text-slate-400 uppercase tracking-tighter italic">How can I help you?</h2>
                            <p class="text-xs text-slate-700 dark:text-slate-600 uppercase tracking-widest mt-2">New thread initialized</p>
                        </div>
                    </div>
                @else
                    @foreach($chat_history as $msg)
                        <div class="flex items-start space-x-6 {{ $msg->role == 'user' ? 'justify-end' : '' }}">
                            ...
                            <div class="relative group max-w-[80%]">
                                <div class="px-5 py-3 rounded-2xl transition-all {{ $msg->role == 'user' ? 'bg-pink-600/10 border border-pink-500/30 text-white shadow-xl shadow-pink-900/5' : 'bg-white/5 text-white' }}">
                ...
                <form action="{{ route('chat') }}" method="POST" id="chatForm">
                    ...
                    <div class="relative flex items-center">
                        <input type="text" name="message" id="chatInput" required autocomplete="off" 
                               class="w-full bg-slate-900 border border-white/10 rounded-2xl px-6 py-5 pr-16 focus:outline-none focus:ring-2 focus:ring-pink-600/50 focus:border-pink-600 transition-all shadow-2xl placeholder-slate-600 text-white" 
    ...
    <div id="notifModal" class="fixed inset-0 z-50 hidden bg-slate-950/80 backdrop-blur-md transition-all duration-300">
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-slate-950 border border-white/10 w-full max-w-lg rounded-3xl shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="notifContainer">
                <div class="p-6 border-b border-white/10 flex items-center justify-between">
                    ...
                    <button id="closeModal" class="p-2 hover:bg-white/5 rounded-xl text-slate-500 hover:text-white transition-colors">
                ...
                <div class="p-6 bg-slate-100 dark:bg-slate-900 border-t border-pink-200 dark:border-slate-800 text-center">
        ...
        const appendMessage = (role, message) => {
            ...
            const html = `
                <div class="flex items-start space-x-6 ${isUser ? 'justify-end' : ''} animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ${!isUser ? '<div class="w-9 h-9 rounded-xl bg-pink-600 flex items-center justify-center text-slate-900 dark:text-white text-[11px] font-black flex-shrink-0 shadow-lg shadow-pink-600/30">JD</div>' : ''}
                    <div class="relative group max-w-[80%]">
                        <div class="px-5 py-3 rounded-2xl transition-all ${isUser ? 'bg-pink-600/10 border border-pink-500/30 text-slate-100 shadow-xl shadow-pink-900/10' : 'bg-transparent text-slate-700 dark:text-slate-300'}">
                            <p class="leading-relaxed text-sm whitespace-pre-wrap">${message}</p>
                        </div>
                        <span class="absolute -bottom-6 ${isUser ? 'right-2' : 'left-2'} text-[9px] text-slate-700 dark:text-slate-600 font-bold uppercase tracking-tighter opacity-100">
                            Just now
                        </span>
                    </div>
            ...
            try {
                ...
                if (data.length > 0) {
                    ...
                    list.innerHTML = data.map(n => `
                        <div class="p-4 bg-slate-800/20 hover:bg-pink-600/5 border border-pink-200 dark:border-slate-800 rounded-2xl transition-all">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-[9px] font-black text-pink-500 uppercase tracking-widest">${n.origen}</span>
                                <span class="text-[9px] text-slate-700 dark:text-slate-600 font-bold">${new Date(n.created_at).toLocaleDateString()}</span>
                            </div>
