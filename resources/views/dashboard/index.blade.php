    ...
    <main class="flex-1 overflow-auto p-6 md:p-10">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <a href="{{ $category['url'] }}" target="_blank" rel="noopener noreferrer" class="group relative bg-slate-950 border border-slate-800 rounded-3xl p-8 transition-all duration-300 hover:scale-[1.03] hover:border-pink-500/50 hover:shadow-2xl hover:shadow-pink-500/10 active:scale-100">
                @else
                <a href="{{ route($category['route']) }}" class="group relative bg-slate-950 border border-slate-800 rounded-3xl p-8 transition-all duration-300 hover:scale-[1.03] hover:border-pink-500/50 hover:shadow-2xl hover:shadow-pink-500/10 active:scale-100">
                @endif
                    <div class="absolute -right-10 -top-10 w-40 h-40 bg-pink-600/5 blur-[60px] group-hover:bg-pink-600/15 transition-all"></div>
                    
                    <div class="w-16 h-16 rounded-2xl bg-pink-500/10 flex items-center justify-center text-3xl mb-6 border border-pink-500/20">
                        {{ $category['icon'] }}
                    </div>

                    <h2 class="text-2xl font-bold text-white mb-3 group-hover:text-pink-400 transition-colors">{{ $category['name'] }}</h2>
                    <p class="text-slate-600 leading-relaxed mb-6">{{ $category['description'] }}</p>

                    <div class="flex items-center text-pink-400 font-bold text-xs uppercase tracking-widest">
