...
<body class="h-full flex flex-col bg-slate-950 text-white overflow-hidden">
    ...
    <!-- Navbar -->
    <nav class="h-20 bg-indigo-900/80 backdrop-blur-md border-b border-indigo-800/50 flex items-center justify-between px-6 sticky top-0 z-30">
    ...
    <main class="flex-1 overflow-auto p-6 md:p-10">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <a href="{{ $category['url'] }}" target="_blank" rel="noopener noreferrer" class="group relative bg-slate-950 border border-slate-800 rounded-3xl p-8 transition-all duration-300 hover:scale-[1.03] hover:border-indigo-500/50 hover:shadow-2xl hover:shadow-indigo-500/10 active:scale-100">
                @else
                <a href="{{ route($category['route']) }}" class="group relative bg-slate-950 border border-slate-800 rounded-3xl p-8 transition-all duration-300 hover:scale-[1.03] hover:border-indigo-500/50 hover:shadow-2xl hover:shadow-indigo-500/10 active:scale-100">
                @endif
    ...
    <div id="profileModal" class="fixed inset-0 z-50 hidden bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-8">
            ...
            <form action="{{ route('dashboard.profile.update') }}" method="POST" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-500 py-3 rounded-xl font-bold transition-all shadow-lg shadow-indigo-600/20 active:scale-95 text-xs uppercase tracking-widest">Update Picture</button>
            </form>
        </div>
    </div>
            ...
            try {
                ...
                if (data.length > 0) {
                    ...
                    list.innerHTML = data.map(n => `
                        <div class="p-3 bg-slate-950 border border-slate-800 rounded-xl hover:border-indigo-500/30 transition-all">
