...
<body class="h-full flex flex-col bg-white dark:bg-slate-950 text-slate-200 overflow-hidden uppercase">
    ...
    <nav class="h-20 bg-slate-100 dark:bg-slate-900/50 backdrop-blur-md border-b border-pink-200 dark:border-slate-800/50 flex items-center justify-between px-6 sticky top-0 z-30">
        <div class="flex items-center space-x-4">
            ...
            <a href="{{ route('dashboard.facebook') }}" class="text-xs font-black text-pink-500 tracking-[0.2em] hover:text-slate-900 dark:text-white transition-colors uppercase font-inter">Facebook</a>
        </div>
        <div class="flex items-center space-x-4">
    ...
    <main class="flex-1 overflow-auto p-8 md:p-12">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                <div>
                    <h1 class="text-4xl font-black text-slate-900 dark:text-white italic tracking-tighter">Facebook <span class="text-pink-500">Post Manager</span></h1>
                    <p class="text-[10px] font-bold text-slate-500 tracking-[0.3em] mt-2 italic">Automated social media orchestration</p>
                </div>
                
                <div class="flex items-center space-x-3">
                    <a href="{{ route('dashboard.facebook.accounts') }}" class="h-[58px] px-6 flex items-center justify-center bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 rounded-2xl hover:border-pink-500/30 transition-all text-slate-700 dark:text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white shadow-2xl active:scale-95 group space-x-3">
                    ...
                    <button onclick="openCreatePostModal()" class="h-[58px] px-8 bg-pink-600 hover:bg-pink-500 text-slate-900 dark:text-white rounded-2xl text-[10px] font-black tracking-[0.2em] transition-all shadow-2xl shadow-pink-600/20 active:scale-95 flex items-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                @endphp
                <div class="group bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 rounded-[2.5rem] overflow-hidden transition-all duration-500 hover:border-pink-500/30 hover:shadow-2xl flex flex-col h-full">
                    <!-- Image Grid Area -->
                    <div class="relative bg-white dark:bg-slate-950 aspect-video overflow-hidden" id="carousel-{{ $post->id }}" data-current="0">
                        @if($imgCount > 0)
                        ...
                        @if($imgCount > 1)
                            <!-- Controls -->
                            <button onclick="moveSlide(event, '{{ $post->id }}', -1, {{ $imgCount }})" class="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 text-slate-900 dark:text-white flex items-center justify-center backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70 z-10">
                            ...
                            <button onclick="moveSlide(event, '{{ $post->id }}', 1, {{ $imgCount }})" class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 text-slate-900 dark:text-white flex items-center justify-center backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70 z-10">
                            ...
                            <div class="absolute bottom-3 left-0 right-0 flex justify-center space-x-1.5 z-10">
                                @foreach($images as $idx => $img)
                                    <div class="w-1.5 h-1.5 rounded-full transition-all {{ $idx == 0 ? 'bg-white dark:bg-slate-950 scale-125' : 'bg-white/40' }}" id="dot-{{ $post->id }}-{{ $idx }}"></div>
                                @endforeach
                            </div>
                        @endif
                        ...
                        <div class="absolute top-4 right-4">
                            ...
                            @endphp
                            <span class="text-[7px] font-black {{ $statusColor }} text-slate-900 dark:text-white px-3 py-1 rounded-full uppercase tracking-[0.2em] shadow-lg">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($post->account)
                                        <span class="text-[8px] font-bold text-slate-700 dark:text-slate-600 dark:text-slate-400 uppercase tracking-widest border border-slate-700/50 px-2 py-0.5 rounded-full">
                                            {{ $post->account->name }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                            <p class="text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed line-clamp-4 font-medium lowercase mb-6">
                                {{ $post->content }}
                            </p>
                        </div>
                            ...
                            <div class="flex flex-col space-y-2">
                                ...
                                <div class="flex items-center space-x-2">
                                    @if($post->status !== 'posted' && $post->status !== 'sent')
                                        <button onclick="editPost({{ json_encode($post) }})" class="w-8 h-8 flex items-center justify-center bg-pink-500/10 text-pink-500 rounded-lg hover:bg-pink-500 hover:text-slate-900 dark:text-white transition-all border border-pink-500/10">
                                    ...
                                    <form action="{{ route('dashboard.facebook.destroy', $post->id) }}" method="POST" onsubmit="return confirm('Abort this post deployment?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-8 h-8 flex items-center justify-center bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-slate-900 dark:text-white transition-all border border-red-500/10">
                ...
                @empty
                <div class="col-span-full py-24 text-center border-2 border-dashed border-pink-200 dark:border-slate-800 rounded-[3rem] opacity-30">
    ...
    <!-- Post Modal -->
    <div id="postModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl p-10 animate-in fade-in zoom-in duration-300">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h2 class="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter mb-1 uppercase">Initialize Deployment</h2>
                    <p class="text-[8px] font-bold text-slate-500 tracking-widest leading-none">Social media content synchronization</p>
                </div>
                <button onclick="document.getElementById('postModal').classList.add('hidden')" class="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-xl text-slate-700 dark:text-slate-600 hover:text-slate-900 dark:text-white transition-all">
            ...
            <form id="createPostForm" class="space-y-6" onsubmit="submitPostToWebhook(event, 'createPostBtn')">
                ...
                <div class="mb-6 animate-in fade-in slide-in-from-top-4">
                    ...
                    <div class="relative group">
                        <textarea id="ai_prompt" rows="2" placeholder="Describe your objective (e.g., 'Generate an engaging promotional post for a new AI course focusing on business automation')" 
                                  class="w-full bg-white dark:bg-slate-950/50 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 text-xs font-medium text-slate-700 dark:text-slate-300 resize-none transition-all placeholder:text-slate-700"></textarea>
                        <button type="button" onclick="generateAIContent()" id="aiBtn" class="absolute right-3 bottom-3 bg-emerald-600 hover:bg-emerald-500 text-slate-900 dark:text-white px-4 py-2 rounded-xl text-[9px] font-black tracking-widest uppercase transition-all shadow-lg active:scale-95 flex items-center space-x-2">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="content" id="create_content" required rows="6" placeholder="What's happening on the network?" 
                                      class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-medium text-slate-200 resize-none lowercase"></textarea>
                        </div>
                        <div>
                            <label class="block text-[9px] font-black text-pink-400 tracking-widest mb-2 uppercase">Target Account</label>
                            <select name="facebook_account_id" class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all appearance-none">
                            ...
                            <div class="relative">
                                ...
                                <div class="relative">
                                    <input type="datetime-local" name="post_at" id="post_at_input"
                                           class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all appearance-none">
                            ...
                            <div>
                                ...
                                <select name="status" class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all appearance-none">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="file" name="image1" accept="image/*"
                                   class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-[10px] font-bold text-slate-500 transition-all">
                        </div>
                        <div>
                ...
                <div class="pt-4">
                    <button id="createPostBtn" type="submit" class="w-full bg-pink-600 hover:bg-pink-500 text-slate-900 dark:text-white font-black py-5 rounded-2xl shadow-2xl shadow-pink-600/20 transition-all active:scale-95 text-[10px] tracking-[0.3em] uppercase">
                        Execute Staging
                    </button>
                </div>
            </form>
    ...
    <!-- AI Generating Modal -->
    <div id="aiGeneratingModal" class="fixed inset-0 z-60 hidden bg-white dark:bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl p-8">
            <div class="flex flex-col items-center justify-center space-y-6">
                ...
                <div class="text-center">
                    <h3 class="text-lg font-black text-slate-900 dark:text-white italic tracking-tighter mb-2">Generando contenido con AI</h3>
                    <p class="text-[9px] font-bold text-slate-700 dark:text-slate-600 dark:text-slate-400 tracking-widest uppercase">AI Deployment Strategy — Por favor espera...</p>
                </div>
    ...
    <!-- Edit Post Modal -->
    <div id="editPostModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl p-10 animate-in fade-in zoom-in duration-300">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h2 class="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter mb-1 uppercase">Modify Deployment</h2>
                    <p class="text-[8px] font-bold text-slate-500 tracking-widest leading-none">Social media content optimization</p>
                </div>
                <button onclick="document.getElementById('editPostModal').classList.add('hidden')" class="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-xl text-slate-700 dark:text-slate-600 hover:text-slate-900 dark:text-white transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </button>
            </div>
            ...
            <form id="editPostForm" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-[9px] font-black text-pink-400 tracking-widest mb-2 uppercase">Target Account</label>
                            <select name="facebook_account_id" id="edit_facebook_account_id" class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all appearance-none">
                                <option value="">Select Account (Optional)</option>
                                @foreach($accounts as $account)
                        ...
                        <div>
                            ...
                            <select name="status" id="edit_status" class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all appearance-none">
                                <option value="scheduled">Scheduled</option>
                                <option value="posted">Posted</option>
                                <option value="cancelled">Cancelled</option>
                            </select>
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full bg-pink-600 hover:bg-pink-500 text-slate-900 dark:text-white font-black py-5 rounded-2xl shadow-2xl shadow-pink-600/20 transition-all active:scale-95 text-[10px] tracking-[0.3em] uppercase">
                        Confirm Changes
                    </button>
                </div>
            </form>
    ...
    <div id="settingsModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-lg rounded-[3rem] overflow-hidden shadow-2xl p-10 animate-in fade-in zoom-in duration-300">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h2 class="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter mb-1 uppercase">Configuration</h2>
                    <p class="text-[8px] font-bold text-slate-500 tracking-widest leading-none">External pipeline orchestration</p>
                </div>
                <button onclick="document.getElementById('settingsModal').classList.add('hidden')" class="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-xl text-slate-700 dark:text-slate-600 hover:text-slate-900 dark:text-white transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </button>
            </div>
            ...
            <form action="{{ route('dashboard.facebook.settings.update') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="url" name="facebook_webhook_url" required value="{{ $webhookUrl }}" 
                           class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all">
    ...
    <div id="profileModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-8">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight">User Profile</h2>
            ...
            <form action="{{ route('dashboard.profile.update') }}" method="POST" class="space-y-4">
                ...
                <div class="flex flex-col items-center mb-4">
                    <div class="w-24 h-24 rounded-full border-2 border-pink-500/20 overflow-hidden bg-white dark:bg-slate-950 mb-4 shadow-2xl">
                    ...
                    <p class="text-[8px] font-bold text-slate-700 dark:text-slate-600 uppercase tracking-widest">Active Identity Asset</p>
                </div>
                <div>
