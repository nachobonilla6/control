...
<body class="h-full flex flex-col bg-white dark:bg-slate-950 text-slate-200 overflow-hidden uppercase">
    ...
    <nav class="h-20 bg-slate-100 dark:bg-slate-900/50 backdrop-blur-md border-b border-pink-200 dark:border-slate-800/50 flex items-center justify-between px-6 sticky top-0 z-30">
        <div class="flex items-center space-x-4">
            ...
            <a href="{{ route('dashboard.facebook') }}" class="text-xs font-black text-slate-500 tracking-[0.2em] hover:text-slate-900 dark:text-white transition-colors uppercase font-inter">Facebook</a>
            <span class="text-slate-800 font-light text-xl italic font-inter">/</span>
    ...
    <div id="profileModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                <div>
                    <h1 class="text-4xl font-black text-slate-900 dark:text-white italic tracking-tighter">Connected <span class="text-pink-500">Facebook Accounts</span></h1>
                    <p class="text-[10px] font-bold text-slate-500 tracking-[0.3em] mt-2 italic">Entity Management Interface</p>
                </div>
                
                <button onclick="document.getElementById('addAccountModal').classList.remove('hidden')" class="px-8 py-4 bg-pink-600 hover:bg-pink-500 text-slate-900 dark:text-white rounded-2xl text-[10px] font-black tracking-[0.2em] transition-all shadow-2xl shadow-pink-600/20 active:scale-95 flex items-center">
            ...
            <!-- Table Card -->
            <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 rounded-[3rem] overflow-hidden shadow-2xl">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-pink-200 dark:border-slate-800/50 bg-slate-800/20">
                            ...
                            <tr class="group hover:bg-pink-500/5 transition-colors duration-300">
                                <td class="px-10 py-6">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 rounded-xl bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 flex items-center justify-center text-xs font-black text-pink-400 group-hover:border-pink-500/50 transition-all">
                                            {{ substr($account->name, 0, 1) }}
                                        </div>
                                        <span class="text-[11px] font-black text-slate-900 dark:text-white tracking-widest lowercase">{{ $account->name }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <span class="text-[10px] font-bold text-slate-700 dark:text-slate-600 dark:text-slate-400 tracking-widest font-inter">#{{ $account->page_id }}</span>
                                </td>
                                <td class="px-8 py-6">
                                    <a href="{{ $account->link }}" target="_blank" class="text-[9px] font-black text-pink-500 hover:text-slate-900 dark:text-white underline underline-offset-4 tracking-[0.1em] lowercase flex items-center group/link">
                                        View Page
                                ...
                                <td class="px-10 py-6 text-right">
                                    <button onclick="openEditModal({{ json_encode($account) }})" class="w-10 h-10 inline-flex items-center justify-center bg-pink-500/10 text-pink-500 rounded-xl hover:bg-pink-500 hover:text-slate-900 dark:text-white transition-all border border-pink-500/10 mr-2 group/edit">
                                    ...
                                    <form action="{{ route('dashboard.facebook.accounts.destroy', $account->id) }}" method="POST" onsubmit="return confirm('Disconnect this account?')" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-slate-900 dark:text-white transition-all border border-red-500/10">
    ...
    <!-- Add Account Modal -->
    <div id="addAccountModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-lg rounded-[3rem] overflow-hidden shadow-2xl p-10 animate-in fade-in zoom-in duration-300">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h2 class="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter mb-1 uppercase">Connect Account</h2>
                    <p class="text-[8px] font-bold text-slate-500 tracking-widest leading-none uppercase">Social graph authentication interface</p>
                </div>
                <button onclick="document.getElementById('addAccountModal').classList.add('hidden')" class="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-xl text-slate-700 dark:text-slate-600 hover:text-slate-900 dark:text-white transition-all">
            ...
            <form action="{{ route('dashboard.facebook.accounts.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="e.g. Josh Dev Official" 
                           class="w-full bg-white dark:bg-slate-950 border border-pink-200 dark:border-slate-800 rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-pink-500/30 text-xs font-bold text-slate-900 dark:text-white transition-all lowercase">
                </div>
                <div>
    ...
    <!-- Edit Account Modal -->
    <div id="editAccountModal" class="fixed inset-0 z-50 hidden bg-white dark:bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="bg-slate-100 dark:bg-slate-900 border border-pink-200 dark:border-slate-800 w-full max-w-lg rounded-[3rem] overflow-hidden shadow-2xl p-10 animate-in fade-in zoom-in duration-300">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h2 class="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter mb-1 uppercase">Edit Account</h2>
                    <p class="text-[8px] font-bold text-slate-500 tracking-widest leading-none uppercase">Modify connection parameters</p>
                </div>
                <button onclick="document.getElementById('editAccountModal').classList.add('hidden')" class="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-xl text-slate-700 dark:text-slate-600 hover:text-slate-900 dark:text-white transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </button>
            </div>
